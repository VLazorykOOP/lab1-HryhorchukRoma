...
void Task1()
{
    ...
    auto it = std::find(std::begin(a), std::end(a), mm);

    auto index = it - std::begin(a);
    if (it != std::end(a)) {
        std::cout <<endl<<"Max Min:"<< mm << endl << " Element found at index " << index << std::endl;
    }
    else {
        std::cout << "Element not found" << std::endl;
    }
    ofstream ifs("rez.txt", ios::app);
    ifs << "Task1: " << mm << ' ' << index << endl;
    ifs.close();
    cout << endl;
    return;
}
...
void Task3()
{
    ...
    int k = 0, n = 0;
    cout << "Enter r:"; cin >> r;
    for (int i = 0; i < A.size(); i += 2, n++)
    {
        if (A[i] * A[i] + A[i + 1] * A[i + 1] <= r * r)
        {
            cout << "Case " << n << ": succesful!" << endl;
            k++;
        }

        else cout << "Case " << n << ": failure!" << endl;
    }
    ofstream ifs("rez.txt", ios::app);
    ifs << "Task3: " << k << endl;
    ifs.close();
